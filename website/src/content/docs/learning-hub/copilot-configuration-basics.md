...
| `defaultMode` | Default agent mode at session start: `agent`, `plan`, or `autopilot` — so you can skip typing `--mode` on every launch (v1.0.81+) |
| `defaultPermissionMode` | Default permission approval mode: `auto`, `interactive`, or `allow-all` — mirrors the `/permissions` picker so your preferred mode is active from the start (v1.0.81+) |
